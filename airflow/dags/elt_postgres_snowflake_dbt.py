...
def elt_postgres_snowflake_dbt():
    ...
    @task(task_id='load_to_s3', multiple_outputs=True)
    def _load_to_s3(table_name: str, **context: dict) -> None:
        """
        Load data from a CSV file to a AWS S3 bucket.
        ...
        try:
            ...
            s3_hook.load_file(filename=f"/tmp/{csv_file_name}", key=f"data/{csv_file_name}", bucket_name=Variable.get("S3_BUCKET_NAME"))
            
            logger.info(f"Successfully load csv file into AWS S3 : s3://{Variable.get('S3_BUCKET_NAME')}/data/{csv_file_name}")
        except Exception as e:
    ...
    def table_group(table_name: str) -> None:
        ...
        extract_to_csv >> load_to_s3 >> load_to_snowflake
